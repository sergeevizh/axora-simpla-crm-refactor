...
class ProductsAdmin extends Axora
{
            ...
            if (!empty($ids)) {
                ...
                case 'add_old_price':
                    ...
                    if($number > 0)
                    {
                        ...
                        $query = $this->db->placehold("UPDATE __variants SET compare_price=price+{$set_sql} WHERE product_id IN(?@)", $ids);
                        $this->db->query($query);
                    }
                    break;
                case 'delete_old_price':
                    $query = $this->db->placehold("UPDATE __variants SET compare_price=0 WHERE product_id IN(?@)", $ids);
                    $this->db->query($query);
                    break;
                case 'move_old_price_to_price_and_clear_her':
