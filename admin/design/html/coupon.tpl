...
{capture name=tabs}
	{if in_array('users', $manager->permissions)}
		<li>
			<a href="index.php?module=UsersAdmin">Покупатели</a>
		</li>
	{/if}
...
<form method="post" id="product" enctype="multipart/form-data">
	...
	<div id="name">
		<input placeholder="" class="name" name="code" type="text" value="{$coupon->code|escape}"/>
	...
	<div id="column_left">
		...
		<div class="block layer">
			<ul>
				<li>
					<label for="coupon_value" class="property">Скидка</label>
					<input id="coupon_value" name="value" class="coupon_value" type="text" value="{$coupon->value|escape}"/>
				...
				<li>
					<label for="min_order_price" class="property">Для заказов от</label>
					<input id="min_order_price" class="coupon_value" type="text" name="min_order_price" value="{$coupon->min_order_price|escape}"> {$currency->sign}
				</li>
				<li>
			...
			<ul>
				<li>
					<label class="property">
						<input type="checkbox" name="expires" value="1"{if $coupon->expire} checked{/if}>
						Истекает
					</label>
