...
{capture name=tabs}
	{if in_array('comments', $manager->permissions)}
		<li>
			<a href="index.php?module=CommentsAdmin">Комментарии <span style="color:red">{if $new_comments_counter > 0}{$new_comments_counter}{/if}</span></a>
		</li>
	{/if}
	<li class="active">
		<a href="index.php?module=FeedbacksAdmin">Обратная связь <span style="color:red">{if $new_feedback_counter > 0}{$new_feedback_counter}{/if}</span></a>
	</li>
{/capture}
...
{if $feedbacks || $keyword}
	<form method="get">
		<div id="search">
			...
			<input class="search" type="text" name="keyword" value="{$keyword|escape}" placeholder=""/>
