...
{if $message_success}
	<div class="message message_success">
		<span class="text">
			{if $message_success=='updated'}Пользователь отредактирован{else}{$message_success|escape}{/if}
		</span>
...
{if $message_error}
	<div class="message message_error">
		<span class="text">
			{if $message_error=='login_exists'}Пользователь с таким email уже зарегистрирован
...
<form method=post id=product>
	...
	<div id="name">
		<input placeholder="" class="name" name=name type="text" value="{$user->name|escape}"/>
		<input name=id type="hidden" value="{$user->id|escape}"/>
		<div class="checkbox">
			<input name="enabled" value="1" type="checkbox" id="active_checkbox"{if $user->enabled} checked{/if}/>
	...
	<div id=column_left>
		...
		<div class="block">
			<ul>
				{if $groups}
					<li>
						<label for="group_id" class="property">Группа</label>
						<select id="group_id" name="group_id">
							<option value='0'>Не входит в группу</option>
							{foreach $groups as $g}
								<option value='{$g->id}'
										{if $user->group_id == $g->id}selected{/if}>{$g->name|escape}</option>
							{/foreach}
						</select>
					</li>
				{/if}
				<li>
					<label for="email" class="property">Email</label>
					<input id="email" name="email" class="simpla_inp" type="text" value="{$user->email|escape}"/>
				</li>
				<li>
					<label for="created" class="property">Дата регистрации</label>
					<input id="created" class="simpla_inp" type="text" disabled value="{$user->created|date}"/>
				</li>
				<li>
					<label for="last_ip" class="property">Последний IP</label>
					<input id="last_ip" class="simpla_inp" type="text" disabled value="{$user->last_ip|escape}"/>
				</li>
			</ul>
		</div>
			...
			<div>
				{foreach $orders as $order}
					<div class="{if $order->paid}green{/if} row">
						...
						<div class="name cell">
							<a href="{url module=OrderAdmin id=$order->id return=$smarty.server.REQUEST_URI}">Заказ
								№{$order->id}</a>
						</div>
						<div class="name cell">
