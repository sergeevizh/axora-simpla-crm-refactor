...
{capture name=tabs}
	<li class="active">
		<a href="{url module=ProductsAdmin keyword=null category_id=null brand_id=null filter=null page=null}">Товары</a>
	</li>
	{if in_array('categories', $manager->permissions)}
		<li><a href="index.php?module=CategoriesAdmin">Категории</a>
		</li>
	{/if}
...
<form method="get">
	<div id="search">
		...
		<input placeholder="" class="search" type="text" name="keyword" value="{$keyword|escape}"/>
							...
							<ul>
								{foreach $product->variants as $variant}
									<li {if !$variant@first}class="variant" style="display:none;"{/if}>
										...
										<input placeholder="" class="stock" type="text" name="stock[{$variant->id}]" value="{if $variant->infinity}∞{else}{$variant->stock}{/if}"/>{$settings->units}
									</li>
								{/foreach}
							</ul>
		...
		<div id="action">
			...
			<span id="select">
				<select name="action">
					...
					{/if}
					<option value="update_price">Изменить цены</option>
			...
			<span id="add_old_price" style="display: none;">
...
<script>
	...
	$(function () {
		...
		// показываем поле для изменения цены
		$("#action select[name=action]").change(function() {
			if($(this).val() == 'update_price')
				$("span#update_price").show();
			else
				$("span#update_price").hide();
		});

		// показываем поле длядобавления старой цены
		$("#action select[name=action]").change(function() {
			if($(this).val() == 'add_old_price')
				$("span#add_old_price").show();
			else
				$("span#add_old_price").hide();
		});
