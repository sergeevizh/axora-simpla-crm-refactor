...
define('PCLZIP_TEMPORARY_DIR', 'admin/files/backup/');


require_once('admin/pclzip/pclzip.lib.php');

class BackupAdmin extends Axora
{

    public function fetch()
    {
        $dir = 'admin/files/backup/';
        // Обработка действий
        if ($this->request->method('post')) {
            switch ($this->request->post('action')) {
                case 'create':
                {
                    $filename = $dir.'admin'.date("Y_m_d_G_i_s").'.zip';
                    ##Дамп базы
                    $this->db->dump($dir.'axora.sql');
                    chmod($dir.'axora.sql', 0777);

                    ### Архивируем
                    $zip = new PclZip($filename);
                    $v_list = $zip->create(array('files', $dir.'axora.sql'), PCLZIP_OPT_REMOVE_PATH, $dir, PCLZIP_CB_PRE_ADD, "myCallBack");
                    if ($v_list == 0) {
                ...
                case 'restore':
                {
                    ...
                    } elseif (!$zip->extract(PCLZIP_OPT_PATH, $dir, PCLZIP_OPT_BY_NAME, 'axora.sql')) {
                        trigger_error('Не могу разархивировать '.$zip->errorInfo(true));
                    } elseif (!is_readable($dir.'axora.sql')) {
                        trigger_error('Не могу прочитать файл /temp/axora.sql');
                    } else {
                        $this->db->restore($dir.'axora.sql');
                        unlink($dir.'axora.sql');
