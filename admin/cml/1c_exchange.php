...
if ($axora->request->get('type') == 'sale' && $axora->request->get('mode') == 'checkauth') {
    print "success\n";
    print session_name()."\n";
    print session_id();
}

if ($axora->request->get('type') == 'sale' && $axora->request->get('mode') == 'init') {
    $tmp_files = glob($dir.'*.*');
    ...
    print "zip=no\n";
    print "file_limit={$axora->config->max_upload_filesize}";
}

if ($axora->request->get('type') == 'sale' && $axora->request->get('mode') == 'file') {
    $filename = $axora->request->get('filename');
    ...
    foreach ($xml->Документ as $xml_order) {
        ...
        $existed_order = $axora->orders->get_order(intval($order->id));
        ...
        if ($existed_order) {
            $axora->orders->update_order($order->id, $order);
        } else {
            $order->id = $axora->orders->add_order($order);
        }
        ...
        foreach ($xml_order->Товары->Товар as $xml_product) {
            ...
            // Ищем товар
            $axora->db->query('SELECT id FROM __products WHERE external_id=?', $product_1c_id);
            $product_id = $axora->db->result('id');
            $axora->db->query('SELECT id FROM __variants WHERE external_id=? AND product_id=?', $variant_1c_id, $product_id);
            $variant_id = $axora->db->result('id');
            ...
            $axora->db->query('SELECT id FROM __purchases WHERE order_id=? AND product_id=? AND variant_id=?', $order->id, $product_id, $variant_id);
            $purchase_id = $axora->db->result('id');
            if (!empty($purchase_id)) {
                $purchase_id = $axora->orders->update_purchase($purchase_id, $purchase);
            } else {
                $purchase_id = $axora->orders->add_purchase($purchase);
            }
            $purchases_ids[] = $purchase_id;
        }
        // Удалим покупки, которых нет в файле
        foreach ($axora->orders->get_purchases(array('order_id'=>intval($order->id))) as $purchase) {
            if (!in_array($purchase->id, $purchases_ids)) {
                $axora->orders->delete_purchase($purchase->id);
            }
        }

        $axora->db->query('UPDATE __orders SET discount=0, total_price=? WHERE id=? LIMIT 1', $xml_order->Сумма, $order->id);
    }


    print "success";
    $axora->settings->last_1c_orders_export_date = date("Y-m-d H:i:s");
}

if ($axora->request->get('type') == 'sale' && $axora->request->get('mode') == 'query') {
    ...
    $filter = array('modified_since'=>$axora->settings->last_1c_orders_export_date);
    $filter['limit'] = $axora->orders->count_orders($filter);

    $orders = $axora->orders->get_orders($filter);
    $currency = $axora->money->get_currency();
    foreach ($orders as $order) {
        ...
        $purchases = $axora->orders->get_purchases(array('order_id'=>intval($order->id)));

        $t1 = $doc->addChild('Товары');
        foreach ($purchases as $purchase) {
            if (!empty($purchase->product_id) && !empty($purchase->variant_id)) {
                $axora->db->query('SELECT external_id FROM __products WHERE id=?', $purchase->product_id);
                $id_p = $axora->db->result('external_id');
                $axora->db->query('SELECT external_id FROM __variants WHERE id=?', $purchase->variant_id);
                $id_v = $axora->db->result('external_id');

                    // Если нет внешнего ключа товара - указываем наш id
                    if (!empty($id_p)) {
                        $id = $id_p;
                    } else {
                        $axora->db->query('UPDATE __products SET external_id=id WHERE id=?', $purchase->product_id);
                    ...
                    if (!empty($id_v)) {
                        ...
                        $axora->db->query('UPDATE __variants SET external_id=id WHERE id=?', $purchase->variant_id);
        ...
        $payment_method = $axora->payment->get_payment_method($order->payment_method_id);
        $delivery = $axora->delivery->get_delivery($order->delivery_id);
...
if ($axora->request->get('type') == 'sale' && $axora->request->get('mode') == 'success') {
    $axora->settings->last_1c_orders_export_date = date("Y-m-d H:i:s");
}


if ($axora->request->get('type') == 'catalog' && $axora->request->get('mode') == 'checkauth') {
    print "success\n";
    print session_name()."\n";
    print session_id();
}

if ($axora->request->get('type') == 'catalog' && $axora->request->get('mode') == 'init') {
    $tmp_files = glob($dir.'*.*');
    if (is_array($tmp_files)) {
        foreach ($tmp_files as $v) {
            unlink($v);
...
if ($axora->request->get('type') == 'catalog' && $axora->request->get('mode') == 'file') {
    $filename = basename($axora->request->get('filename'));
    $f = fopen($dir.$filename, 'ab');
    fwrite($f, file_get_contents('php://input'));
    fclose($f);
    print "success\n";
...
if ($axora->request->get('type') == 'catalog' && $axora->request->get('mode') == 'import') {
    $filename = basename($axora->request->get('filename'));
...
function import_categories($xml, $parent_id = 0)
{
    ...
    if (isset($xml->Группы->Группа)) {
        foreach ($xml->Группы->Группа as $xml_group) {
            $axora->db->query('SELECT id FROM __categories WHERE external_id=?', $xml_group->Ид);
            $category_id = $axora->db->result('id');
            if (empty($category_id)) {
                $category_id = $axora->categories->add_category(array('parent_id'=>$parent_id, 'external_id'=>$xml_group->Ид, 'url'=>translit($xml_group->Наименование), 'name'=>$xml_group->Наименование, 'meta_title'=>$xml_group->Наименование, 'meta_keywords'=>$xml_group->Наименование, 'meta_description'=>$xml_group->Наименование ));
            }
...
function import_features($xml)
{
    ...
    foreach ($property as $xml_feature) {
        ...
        // Иначе обрабатываем как обычной свойство товара
            ...
            $axora->db->query('SELECT id FROM __features WHERE name=?', strval($xml_feature->Наименование));
            $feature_id = $axora->db->result('id');
            if (empty($feature_id)) {
                $feature_id = $axora->features->add_feature(array('name'=>strval($xml_feature->Наименование)));
            }
...
function import_product($xml_product)
{
    ...
    if (empty($product_id) && !empty($variant->sku)) {
        $axora->db->query('SELECT product_id, id FROM __variants WHERE sku=?', $variant->sku);
        $res = $axora->db->result();
        if (!empty($res)) {
    ...
    if (empty($product_id)) {
        ...
        $product_id = $axora->products->add_product(
                                array(
        ...
        if (isset($category_id)) {
            $axora->categories->add_product_category($product_id, $category_id);
        }
        ...
        if (isset($xml_product->Картинка)) {
            foreach ($xml_product->Картинка as $img) {
                $image = basename($img);
                if (!empty($image) && is_file($dir.$image) && is_writable($axora->config->original_images_dir)) {
                    rename($dir.$image, $axora->config->original_images_dir.$image);
                    $axora->products->add_image($product_id, $image);
                }
            }
        }
    }
    //Если нашелся товар
        ...
        if (empty($variant_id) && !empty($variant_1c_id)) {
            ...
            $axora->db->query('SELECT id FROM __variants WHERE product_id=?', $product_id);
            $variant_id = $axora->db->result('id');
        }
        ...
        if ($full_update) {
            ...
            $product_id = $axora->products->update_product($product_id, $p);

            // Обновляем категорию товара
            if (isset($category_id) && !empty($product_id)) {
                $query = $axora->db->placehold('DELETE FROM __products_categories WHERE product_id=?', $product_id);
        ...
        if (isset($xml_product->Картинка)) {
            $axora->db->query('SELECT id FROM __images WHERE product_id=? ORDER BY position', $product_id);
			$img_ids = $axora->db->results('id');
			foreach ($img_ids as $img_id) 
			{
				if(!empty($img_id))
				    $axora->products->delete_image($img_id);
			}
    ...
    if (empty($variant_id)) {
        ...
        $variant->stock = 0;
        $variant_id = $axora->variants->add_variant($variant);
        $axora->variants->update_variant($variant_id, array('position'=>$variant_id));
    } elseif (!empty($variant_id)) {
        $axora->variants->update_variant($variant_id, $variant);
    }
    // Свойства товара
        ...
        foreach ($xml_product->ЗначенияСвойств->ЗначенияСвойства as $xml_option) {
            if (isset($_SESSION['features_mapping'][strval($xml_option->Ид)])) {
                ...
                if (isset($category_id) && !empty($feature_id)) {
                    $axora->features->add_feature_category($feature_id, $category_id);
                    $values = array();
                    ...
                    $axora->features->update_option($product_id, $feature_id, implode(' ,', $values));
                }
            }
            ...
            elseif (isset($_SESSION['brand_option_id']) && !empty($xml_option->Значение)) {
                ...
                // Добавим бренд
                // Найдем его по имени
                $axora->db->query('SELECT id FROM __brands WHERE name=?', $brand_name);
                if (!$brand_id = $axora->db->result('id')) {
                    // Создадим, если не найден
                    $brand_id = $axora->brands->add_brand(array('name'=>$brand_name, 'meta_title'=>$brand_name, 'meta_keywords'=>$brand_name, 'meta_description'=>$brand_name, 'url'=>translit($brand_name)));
                }
                if (!empty($brand_id)) {
                    $axora->products->update_product($product_id, array('brand_id'=>$brand_id));
                }
            }
        }
    }
    ...
    if ($xml_product->Статус == 'Удален') {
        $axora->variants->delete_variant($variant_id);
        $axora->db->query('SELECT count(id) as variants_num FROM __variants WHERE product_id=?', $product_id);
        if ($axora->db->result('variants_num') == 0) {
            $axora->products->delete_product($product_id);
        }
    }
}
...
function import_variant($xml_variant)
{
    ...
    $axora->db->query('SELECT v.id FROM __variants v WHERE v.external_id=? AND product_id=(SELECT p.id FROM __products p WHERE p.external_id=? LIMIT 1)', $variant_1c_id, $product_1c_id);
    $variant_id = $axora->db->result('id');

    $axora->db->query('SELECT p.id FROM __products p WHERE p.external_id=?', $product_1c_id);
    $variant->external_id = $variant_1c_id;
    $variant->product_id = $axora->db->result('id');
    ...
    if (!empty($xml_variant->Цены->Цена->Валюта)) {
        // Ищем валюту по коду
        $axora->db->query("SELECT id, rate_from, rate_to FROM __currencies WHERE code like ?", $xml_variant->Цены->Цена->Валюта);
        $variant_currency = $axora->db->result();
        // Если не нашли - ищем по обозначению
        if (empty($variant_currency)) {
            $axora->db->query("SELECT id, rate_from, rate_to FROM __currencies WHERE sign like ?", $xml_variant->Цены->Цена->Валюта);
            $variant_currency = $axora->db->result();
        }
